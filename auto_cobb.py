...
# -*- coding: utf-8 -*-
# ядро авто-измерения угла Cobb из одиночного рентгеновского снимка (градации серого).
# нормализует яркость, ищет центрлайн позвоночного столба внутри центрального вертикального ROI
# по производной центрлайна оценивает наклоны «верхнего» и «нижнего» сегментов и считает угол Cobb
# формирует красивый оверлей с ROI, центрлайном и отметками уровней.
...
def _smooth_1d(x: np.ndarray,                                                                           #сглаживает 1D массив
    ...
    if _HAS_SG:
        ...
        k = _gauss1d_kernel(half, sigma)                                                                # гауссовая свёртка собственным ядром _gauss1d_kernel.
